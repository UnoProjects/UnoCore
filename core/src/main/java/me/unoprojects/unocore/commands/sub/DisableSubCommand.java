...
public class DisableSubCommand extends SubCommand<UnoCore> {
    ...
    protected void setup() {
        ...
        executes((sender, args) -> {
            ...
            if (!module.isEnabled()) {
                sender.sendMessage(parse(PREFIX + "<red>Il modulo <yellow><b>" + moduleName + "</b></yellow> è già disabilitato."));
                return;
            }

            Bukkit.getPluginManager().disablePlugin(module);
            sender.sendMessage(parse(PREFIX + "Il modulo <yellow><b>" + moduleName + "</b></yellow> è stato <red>disabilitato</red> con successo."));
        });
    }
}
